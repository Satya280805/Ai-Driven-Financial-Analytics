...
🎯 Objectives
Analyze enterprise-style financial transaction data.
Perform data cleaning and preprocessing.
Conduct Exploratory Data Analysis (EDA).
Engineer financial and time-based features.
Analyze revenue, expenses, budgets, departments, and account categories.
Develop an expense forecasting model.
Forecast monthly expenses for 2026.
Detect potentially anomalous financial transactions.
Evaluate Machine Learning models using suitable metrics.
Develop interactive financial dashboards using Power BI.
Develop a Flask-based web application for financial analytics.
Present insights in a form useful for financial decision support.
...
Transaction_ID	Financial transaction reference
Transaction_Date	Posting / transaction date
Company_Code	Company Code
Cost_Center	Cost Center
Profit_Center	Profit Center
Account_Category	Financial account/category
...
Anomaly_Indicator	Reference anomaly label
📊 Dataset
...
Steps Performed
Loaded the financial transaction dataset using pandas.
Inspected data types and dataset structure.
Checked missing values.
Checked duplicate transaction IDs.
Standardized categorical values.
Removed unnecessary whitespace from categorical fields.
Standardized company codes.
Converted transaction dates into proper datetime format.
Handled missing Quantity values using median imputation.
Preserved meaningful missing values in fields such as Invoice_ID, Vendor_ID, and Profit.
Missing-Value Handling

The remaining missing values have business meaning:

Invoice_ID: 2,153 missing
Vendor_ID: 4,695 missing
...
requirements.txt
🔍 Key Findings
Total financial transactions analyzed: 9,802.
Total revenue was approximately ₹993.38M.
Total expense was approximately ₹1.64B.
Expenses increased from ₹244.92M in 2021 to ₹429.15M in 2025.
Operations and IT were the highest-spending departments.
Procurement was the largest expense category.
2025 expense exceeded its budget by approximately ₹19.58M.
Gradient Boosting performed best among the tested forecasting models.
Forecasted 2026 expense is approximately ₹490.87M.
Isolation Forest identified 197 potential anomalies.
The anomaly detector achieved an F1-score of 36.73% against the reference labels.
Power BI and Flask provide complementary interfaces for financial insights.
⚠️ Limitations
The financial dataset is synthetically generated.
The dataset does not represent live SAP S/4HANA production data.
The monthly forecasting dataset contains only 60 months, with 36 training observations after feature engineering.
Forecasting performance may change with larger real-world datasets.
Isolation Forest detects statistical irregularities and does not establish fraud.
Anomaly detection performance can be improved with additional domain-specific features.
SAP integration is represented through enterprise concepts rather than a live transactional integration.
🚀 Future Scope
